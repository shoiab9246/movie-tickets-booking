...
const CitySearch = () => {
    ...
    const [cityTheatres, setCityTheatres] = React.useState({});

    const getTheatres = async(city_name, state_name) => {
        const requestOptions = {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ city: city_name, state: state_name })
        };
        fetch(`${API_URL}/theatre/theatrebycity`, requestOptions)
        .then(
            async response => {
                const theatres = await response.json();
                setCityTheatres(theatres);
            }
        )
    }
    ...
    const handleSelect = async (value, placeId) => {
        ...
        setState(state)
        getTheatres(locality,state)

    };

    const [value, setValue] = React.useState('us');
    var searchOptions = {componentRestrictions: {country: value}};
    ...
    return (  
             ...
             <div>
                 ...
                 <div>
                     ...
                     {suggestions.map((suggestion) => {
                         const style = {
                             backgroundColor: suggestion.active ? "aliceblue" : "#fff"
                         }
        ...
        <div style={{textAlign: 'center'}}>
            <TheatrePage theatres={cityTheatres} ></TheatrePage>
        </div>
        {/* <h2><Link to={ROUTES.SEARCH}>go to search page for demo</Link></h2> */}
